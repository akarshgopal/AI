...
def init_matrix(n1, n2, activation):
    ...
    if isinstance(activation, (sigmoid, softmax)):
        M = (np.random.randn(n1, n2) - 0.5) * np.sqrt(2.0 / n1)
    elif isinstance(activation, (relu, leaky_relu)):
        M = (np.random.randn(n1, n2) - 0.5) * np.sqrt(1.0 / n1)
    elif isinstance(activation, tanh):
        M = (np.random.randn(n1, n2) - 0.5) * np.sqrt(1.0 / (n1 + n2))
    else:
...
def SGD(batch_size, X, Y, model, lr, beta, reg_lambda=0):
    m = np.shape(X)[0]
    H = np.zeros(Y.shape)
    for i in tqdm(range(0, m, batch_size), desc="Descending gradients", unit="batch"):
...
class genetic_optimizer:
    def __init__(
        ...
        self.population = np.random.uniform(
            low=-4.0, high=4.0, size=(self.num_genes, self.population_size)
        )
    ...
    def next_generation(self):
        ...
        while k < self.population_size:
            if k < 2 * N:
                self.population[:, k] = self.crossover(
                    self.population[:, k - N], self.population[:, k - N + 1]
                )

            if (k >= 2 * N) and (k < 3 * N):
                m = int(4 * N * np.random.uniform())
                self.population[:, k] = self.crossover(
                    self.population[:, k - 2 * N], self.population[:, m]
                )
            ...
            if k >= 3 * N:
                ...
                self.population[:, k] = self.crossover(
                    self.population[:, m1], self.population[:, m2]
                )
...
def train(
    ...
    data_size = X.shape[0]
    pbar = tqdm(range(n_epochs), desc="Training, lifting weights", unit="epoch")
    for e in pbar:
        # shuffle dataset
        np.random.seed(138)
        ...
        if show_test_acc:
            ...
            acc = 0
        pbar.set_postfix({"loss": loss, "train accuracy": tr_acc, "test accuracy": acc})
...
class layer:
    ...
    def grad(self, dA, A0, m):
        ...
        self.dW = (1.0 / m) * np.einsum("mn,ml->ln", self.dZ, A0)
        self.dB = (1.0 / m) * (np.einsum("mn->n", self.dZ))
    ...
    def step(self, lr, beta):
        self.V_dW = beta * self.V_dW + (1.0 - beta) * self.dW
        self.V_dB = beta * self.V_dB + (1.0 - beta) * self.dB
...
class conv_layer:
    def __init__(
        ...
        self.W = init_filters(
            self.n_filters, self.n_channels, self.kernel_size, activation
        )
    ...
    def grad(self, dA):
        ...
        self.dW = (1.0 / batch_size) * np.einsum("mjk,mkl->jl", self.im2cols, self.dZ)
        self.dB = (1.0 / batch_size) * np.einsum("mjk->k", self.dZ)

        dA_prev_cols = np.einsum("mnk,fk-> mfn", self.dZ, self.W)

        dA_prev = np.zeros(self.A_prev_shape)
        for i in range(self.act_idx.shape[-1]):
            dA_prev.ravel()[self.act_idx[:, :, i].ravel()] += dA_prev_cols[
                :, :, i
            ].ravel()

        dA_prev = dA_prev.reshape(
            dA_prev.shape[0], dA_prev.shape[1], dA_prev.shape[2] * dA_prev.shape[3]
        )
...
class max_pool_layer:
    ...
    def grad(self, dA):
        ...
        A = self.A.reshape(
            self.A.shape[0], self.A.shape[1], 1, self.A.shape[-2] * self.A.shape[-1]
        )
        dAdZ = (self.Z - A) == 0
        self.dZ = np.multiply(dA, dAdZ)
        shape = self.dZ.shape
